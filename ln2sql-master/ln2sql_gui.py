...
class App:
	def __init__(self, root):
		...
		self.load_language_button = Button(self.language_frame, text="Choose a configuration file file", command=self.find_csv_file, width=20)
	...
	def find_sql_file(self):
		filename = filedialog.askopenfilename(title="Select a SQL file",filetypes=[('sql files','.sql'),('all files','.*')])
		self.database_path_label["text"] = filename

	def find_thesaurus_file(self):
		filename = filedialog.askopenfilename(title="Select a thesaurus file",filetypes=[('thesaurus files','.dat'),('all files','.*')])
		self.thesaurus_path_label["text"] = filename

	def find_csv_file(self):
		filename = filedialog.askopenfilename(title="Select a language configuration file",filetypes=[('csv files','.csv'),('all files','.*')])
	...
	def lanch_parsing(self):
		try:
			...
			if (str(self.database_path_label["text"]) != "No SQL dump selected...") and (str(self.language_path_label["text"]) != "No configuration file selected...") and (str(self.input_sentence_string.get()) != "Enter a sentence..."):
				ln2sql(self.database_path_label["text"], self.input_sentence_string.get(), self.language_path_label["text"], thesaurus_path, './output.json')
